...
def hierarchical_planner_bootstrap(
    seed_to_task_differences: Mapping[str | int, Iterable[float]],
    *,
    resamples: int = 10000,
    seed: int = BOOTSTRAP_SEED,
) -> tuple[float, float, float]:
    groups = {
        str(k): np.asarray([float(v) for v in vals], dtype=float)
        for k, vals in seed_to_task_differences.items()
    }
    ...
    keys = sorted(groups)
    observed = float(np.mean([groups[k].mean() for k in keys]))
    if (
        all(np.allclose(groups[k], groups[k][0]) for k in keys)
        and len({float(groups[k][0]) for k in keys}) == 1
    ):
        return observed, observed, observed
    rng = np.random.default_rng(seed)
...
def clustered_stage1a_bootstrap(
    task_to_snapshot_differences: Mapping[str, Iterable[float]],
    *,
    resamples: int = 10000,
    seed: int = BOOTSTRAP_SEED,
) -> tuple[float, float, float]:
...
def paired_tost(
    differences: Iterable[float], margin: float, *, alpha: float = 0.05
) -> dict[str, float | bool]:
    ...
    return {
        ...
        "p_lower": p_lower,
        "p_upper": p_upper,
        "pass": p_lower < alpha and p_upper < alpha,
    }
...
def evaluate_gate(
    ...
    threshold: float,
    *,
    raw_differences: Iterable[float] | None = None,
) -> bool:
    if rule == "ci_low_gte":
        return ci_low >= threshold
    if rule == "ci_low_gt":
        return ci_low > threshold
    if rule == "estimate_gte_and_ci_low_gt":
        return estimate >= threshold and ci_low > 0
    if rule == "estimate_gte_and_ci_low_gte":
        return estimate >= 0 and ci_low >= threshold
    if rule == "estimate_gte":
        return estimate >= threshold
    if rule == "estimate_lte":
        return estimate <= threshold
    if rule == "minimum_positive_seed_count":
        return estimate >= threshold
    if rule == "paired_tost":
        if raw_differences is None:
            raise ValueError("paired_tost requires raw_differences")
        return bool(paired_tost(raw_differences, threshold)["pass"])
    raise ValueError(f"unknown locked rule: {rule}")


def stage_decision(
    stage: str, gates: list[dict[str, Any]], *, planner_stage1b_eligible: bool | None = None
) -> str:
    ...
    if stage == "PLANNER":
        ...
        if not architecture or not all(g["pass"] for g in architecture):
            return "STOP_PLANNER"
        return (
            "GO_PLANNER_STAGE1B_ELIGIBLE"
            if eligibility and all(g["pass"] for g in eligibility)
            else "GO_PLANNER_DIAGNOSTIC_ONLY"
        )
    if stage == "STAGE1A":
        if not passed:
            return "STOP_INTERFACE"
        return (
            "GO_INTERFACE_STAGE1B_ELIGIBLE"
            if planner_stage1b_eligible
            else "GO_INTERFACE_DIAGNOSTIC_ONLY"
        )
    if stage == "STAGE1B":
        return "GO_END_TO_END" if passed else "STOP_END_TO_END"
