...
совпадении train-only task IDs, `dataset_lineage_order`,
`optimizer_execution_task_order` и обоих versioned bindings.
...
- seeds `17`, `29`, `43`;
- `3` epochs;
- dataset lineage order: `1, 3, 2`;
- фактический optimizer execution order, полученный сортировкой по `task_id`: `1, 2, 3`;
- `9` updates/run;
...
- frozen dataset lineage, dataset lineage order и отдельный optimizer execution order;
...
`validate_diagnostic(root)` также требует точный top-level file set и не принимает
необъявленные файлы вне `training-runs`.
