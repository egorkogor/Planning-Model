...
def _pooled_task_teacher_summary_from_raw(tasks: list[dict[str, Any]]) -> dict[str, Any]:
    positions = [position for task in tasks for position in task["positions"]]
    non_end = [row for row in positions if row["gold_operator"] != "END"]
    end = [row for row in positions if row["gold_operator"] == "END"]
    arg1 = [row for row in positions if row["has_arg1_target"]]
...
def _aggregate_teacher_summary_from_raw(tasks: list[dict[str, Any]]) -> dict[str, Any]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for task in tasks:
        grouped.setdefault(task["task_id"], []).append(task)
    if set(grouped) != set(EXPECTED_TRAIN_TASK_IDS):
        raise ValueError("A2_BUDGET_VALIDATOR_AGGREGATE_TASK_COVERAGE")
    ...
    for task_id in EXPECTED_TRAIN_TASK_IDS:
        ...
        if len(seeds) != len(set(seeds)):
            raise ValueError(f"A2_BUDGET_VALIDATOR_AGGREGATE_DUPLICATE_SEED:{task_id}")
        ...
        if len(gold_operators) != 1:
            raise ValueError(f"A2_BUDGET_VALIDATOR_AGGREGATE_GOLD:{task_id}")
    ...
    return {
        ...
        "per_task": {
            task_id: _pooled_task_teacher_summary_from_raw(grouped[task_id])
            for task_id in EXPECTED_TRAIN_TASK_IDS
        },
        "position0_by_task": position0_by_task,
        "position0_unstack": {
...
def _validate_aggregate_teacher_claim(
    tasks: list[dict[str, Any]], claimed: dict[str, Any], *, epoch: int
) -> dict[str, Any]:
    expected = _aggregate_teacher_summary_from_raw(tasks)
    if claimed != expected:
        raise ValueError(f"A2_BUDGET_VALIDATOR_AGGREGATE_TEACHER_CLAIM:{epoch}")
    return expected
...
def _aggregate_checkpoint_from_raw(
    ...
    *,
    ...
    for result in seed_results:
        ...
        seed_teacher = checkpoint["teacher_forced"]
        teacher.extend(seed_teacher)
        free.extend(checkpoint["free_running"])
        discriminations.append(
            _teacher_summary_from_raw(seed_teacher)["position0_task_discrimination"]
        )
    teacher_summary = _aggregate_teacher_summary_from_raw(teacher)
    return {
        "epoch": epoch,
