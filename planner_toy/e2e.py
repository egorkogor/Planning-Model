...
GENERATION_FAILURE_CODES = FAILURE_CODES - {
    "EXECUTOR_PRECONDITION_FAILED", "GOAL_NOT_ACHIEVED",
    "SEMANTIC_TRACE_MISSING", "SEMANTIC_TRACE_LENGTH", "VARIANT_MISMATCH",
}
    ...
    *, request: dict, work_plan: dict | None, attempts: list[dict], manifest: dict,
    ...
    if semantic_trace is not None:
        trace_semantic = {
            ...
            if key not in {"checkpoint_file_hash", "planner_request_hash"}
        }
    return toy_hash(
        "quality_evidence_semantic",
        {
            "request": request, "work_plan": work_plan, "attempts": attempts,
...
def validate_persisted_quality_evidence(
    *, root: Path, task: dict, checkpoint: dict
    ...
    try:
        ...
        actual_goal_success = goal_satisfied(state, tuple(tuple(x) for x in task["goal"]))
        ...
        if (
            ...
            or episode["goal_success"] != actual_goal_success
            or evaluation["success"] != actual_goal_success
        ):
