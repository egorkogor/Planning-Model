...
SCHEMA_PATH = Path(__file__).parent / "schemas" / "learnability_diagnostic.schema.json"
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        **provenance,
        ...
        "dataset_lineage_order": list(dataset["train_task_ids"]),
        "optimizer_execution_task_order": [row["task_id"] for row in train_rows],
...
def validate_payload(payload: dict[str, Any], *, root: Path | None = None) -> None:
    ...
    dataset, train_rows = _dataset_context()
    train_by_id = {row["task_id"]: row for row in train_rows}
    if payload["dataset_lineage_order"] != dataset["train_task_ids"]:
        raise ValueError("LEARNABILITY_DATASET_LINEAGE_ORDER_MISMATCH")
    optimizer_order = [row["task_id"] for row in train_rows]
    if payload["optimizer_execution_task_order"] != optimizer_order:
        raise ValueError("LEARNABILITY_OPTIMIZER_EXECUTION_ORDER_MISMATCH")
    if (
        payload["frozen_dataset_lineage_hash"]
