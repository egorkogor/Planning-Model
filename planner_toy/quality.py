...
def canonical_replay_payload(root: Path, manifest: dict, rows: list[dict]) -> dict:
    """Cross-worker semantic identity; raw hashes remain local-manifest integrity only."""
    checkpoint_identities = {}
    for path in sorted(root.glob("training-runs/*/seed-*/checkpoint-manifest.json")):
        checkpoint = json.loads(path.read_bytes())
        checkpoint_identities[str(path.relative_to(root))] = {
            "initialization": checkpoint["canonical_initialization_state_dict_sha256"],
            "trained": checkpoint["canonical_trained_state_dict_sha256"],
            "optimizer": checkpoint["canonical_optimizer_state_sha256"],
        }
    config = json.loads((root / "evaluation-config.json").read_bytes())
    dataset = json.loads((root / "dataset-manifest.json").read_bytes())
    per_seed = json.loads((root / "per-seed-summary.json").read_bytes())
    aggregate = json.loads((root / "aggregate-summary.json").read_bytes())
    paired = json.loads((root / "paired-comparisons.json").read_bytes())
    semantic_rows = task_results_semantic_projection(rows)
    return {
        "schema_version": "toy-quality-canonical-replay/2.0",
        ...
        "config_semantic_hash": sha256(config),
        "dataset_semantic_hash": sha256(dataset),
        "task_results_semantic_hash": sha256(semantic_rows),
        "per_seed_semantic_hash": sha256(per_seed),
        "aggregate_semantic_hash": sha256(aggregate),
        "paired_comparisons_semantic_hash": sha256(paired),
...
def task_results_semantic_projection(rows: list[dict]) -> list[dict]:
    return [
        {key: value for key, value in row.items() if key != "checkpoint_hash"}
        for row in rows
    ]
...
def _evaluate(row: dict, variant: str, seed: int, model: LockedPlanner, checkpoint: dict, evidence_root: Path) -> dict:
    ...
    return {
        ...
        "checkpoint_hash": checkpoint["trained_state_dict_sha256"],
        "canonical_checkpoint_hash": checkpoint["canonical_trained_state_dict_sha256"],
...
def validate_evaluation(root: Path, *, force_training_replay: bool = False) -> dict:
    ...
    config = json.loads((root / "evaluation-config.json").read_bytes())
    if config.get("canonical_cpu_runtime") != runtime_fingerprint:
        raise ValueError("CANONICAL_CPU_RUNTIME_PROFILE_MISMATCH")
    validate_schema("toy_quality_evaluation.schema.json", config)
    if config["canonical_numeric_policy"] != CANONICAL_NUMERIC_POLICY:
        raise ValueError("CANONICAL_NUMERIC_POLICY_MISMATCH")
    ...
    expected_artifacts = {"evaluation-config.json", "dataset-manifest.json", "task-results.jsonl", "task-results-semantic.json", "canonical-semantic-payload.json", "per-seed-summary.json", "aggregate-summary.json", "paired-comparisons.json", "human-readable-examples.md"}
    ...
    if json.loads((root / "task-results-semantic.json").read_bytes()) != (
        task_results_semantic_projection(rows)
    ):
        raise ValueError("TASK_RESULTS_SEMANTIC_PROJECTION_MISMATCH")
    if json.loads((root / "canonical-semantic-payload.json").read_bytes()) != (
        canonical_replay_payload(root, manifest, rows)
    ):
        raise ValueError("CANONICAL_SEMANTIC_PAYLOAD_MISMATCH")
...
def export_compact(root: Path, destination: Path) -> None:
    ...
    compact = {
        ...
        "canonical_numeric_policy": config["canonical_numeric_policy"],
...
def run(
    ...
    *,
    ...
    _write(output / "task-results-semantic.json", task_results_semantic_projection(rows))
    names = ["evaluation-config.json", "dataset-manifest.json", "task-results.jsonl", "task-results-semantic.json", "per-seed-summary.json", "aggregate-summary.json", "paired-comparisons.json", "human-readable-examples.md"]
    ...
    _write(
        ...
        canonical_replay_payload(output, manifest, rows),
    )
    manifest["artifact_hashes"]["canonical-semantic-payload.json"] = _file_hash(
        output / "canonical-semantic-payload.json"
    )
    _write(output / "evaluation-manifest.json", manifest)
