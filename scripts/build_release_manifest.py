...
def excluded(rel):
    return any(fnmatch.fnmatch(rel, p) for p in EXCLUDED)
...
def files():
    lock = yaml.safe_load(
        (ROOT / "docs/operator/scientific_lock_v1.yaml").read_text(encoding="utf-8")
    )
    ...
    for pattern in patterns:
        rows.extend(expand(pattern))
    return sorted(
        {p for p in rows if not excluded(p.relative_to(ROOT).as_posix())},
...
def main():
    ...
    obj = {
        "schema_version": "work-planner-release/1.1",
        "release_id": "work-planner-v1.21-v2.21",
        "implementation_spec": "1.21",
        "stage1_runbook": "2.21",
        "generated_at": generated,
        "scope": "SCIENTIFIC_TRUST_ROOT_ONLY",
        "files": {p.relative_to(ROOT).as_posix(): digest(p) for p in files()},
        "excluded_paths": EXCLUDED,
        "manifest_hash": "",
    }
    obj["manifest_hash"] = canonical(obj)
