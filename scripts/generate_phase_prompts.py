...
'P18':'P18_end_to_end_decision.md','P19':'P19_independent_audit_and_reproduction.md','P20':'P20_final_acceptance.md'}
COMMON=[
'docs/operator/agent_execution_contract_v1.yaml','docs/operator/phase_state_machine_v1.yaml','docs/operator/phase_registry_v1.yaml',
'docs/operator/report_registry_v1.yaml','docs/operator/self_review_loop_v1.yaml']


def bullets(rows): return '\n'.join(f'- `{x}`' for x in rows) if rows else '- нет'
def checks(rows): return '\n'.join(f"- `{x['check_id']}` — {x['description']}" + (f"; verifier: `{x['verifier']}`" if x.get('verifier') else '') for x in rows) if rows else '- нет'

def conditional_rows(mapping):
    if not mapping: return '- нет'
    out=[]
    for outcome, rows in mapping.items():
        out.append(f'- `{outcome}`:')
        out.extend(f'  - `{row}`' if isinstance(row,str) else f"  - `{row['check_id']}` — {row['description']}" for row in rows)
    return '\n'.join(out)


def render(phase,transitions):
    pid=phase['phase_id']; gate=phase.get('manual_gate_id') or 'нет'; role=phase['execution_role']
    outputs=list(phase.get('required_outputs',[])); pre=list(phase.get('pre_gate_required_outputs',[]) or []); post=list(phase.get('post_gate_required_outputs',[]) or []); inputs=list(phase.get('required_inputs',[]) or [])
    outcome_lines=[]
...
def main():
    reg=yaml.safe_load(REGISTRY.read_text(encoding='utf-8')); sm=yaml.safe_load(STATE.read_text(encoding='utf-8'))
