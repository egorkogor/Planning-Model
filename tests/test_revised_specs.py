...
def test_action_contracts_reject_bad_signatures() -> None:
    ...
    valid_end = {"schema_version": "work-planner/1.16", "action": "END", "args": []}
    assert not list(validator.iter_errors(valid_end))
    assert list(validator.iter_errors({"schema_version": "work-planner/1.16", "action": "END", "args": ["block_0"]}))
    assert list(validator.iter_errors({"schema_version": "work-planner/1.16", "action": "PICK_UP", "args": []}))
