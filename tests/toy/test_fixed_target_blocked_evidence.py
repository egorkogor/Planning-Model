...
from scripts.fixed_target_contract import validate_acceptance
...
def test_committed_blocked_acceptance_record_is_valid() -> None:
    ...
    assert value["cross_attempt_comparison"] == {"mismatches": [], "status": "NOT_RUN"}
    assert value["target_contract"] is None
    assert value["runtime_contract"] is None


def test_fixed_target_workflow_is_controlled_preflight_only() -> None:
