...
def test_train_only_never_builds_heldout_tasks(monkeypatch) -> None:
    import planner_toy.dataset as dataset_module

    calls: list[int] = []
    original = dataset_module._build_task_row

    def observed(serial: int):
        calls.append(serial)
        return original(serial)

    monkeypatch.setattr(dataset_module, "_build_task_row", observed)
...
def test_heldout_definition_mutation_does_not_change_train_hash(monkeypatch) -> None:
    ...
    mutated = dict(dataset_module._TASK_SPECS_V1)
    mutated[4] = (3, (1, 0, 2))
    mutated[5] = (3, (0, 2, 1))
    monkeypatch.setattr(dataset_module, "_TASK_SPECS_V1", mutated)
...
def test_train_row_mutation_changes_train_hash(monkeypatch) -> None:
    ...
    monkeypatch.setattr(dataset_module, "_build_task_row", mutated)
    after = generate_train_only(17)["evaluated_train_split_hash"]
    assert after != before
