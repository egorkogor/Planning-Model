...
def test_independent_process_runs_have_stable_canonical_identity(tmp_path: Path) -> None:
    ...
    for root, hash_seed in zip(roots, ("101", "202"), strict=True):
    ...
    checkpoint_relative = "training-runs/A2/seed-17/checkpoint-manifest.json"
    checkpoints = [json.loads((root / checkpoint_relative).read_text()) for root in roots]
    for field in (
        "canonical_initialization_state_dict_sha256",
        "canonical_trained_state_dict_sha256",
        "canonical_optimizer_state_sha256",
    ):
        assert checkpoints[0][field] == checkpoints[1][field]
    configs = [json.loads((root / "evaluation-config.json").read_text()) for root in roots]
    assert configs[0]["canonical_cpu_runtime"] == configs[1]["canonical_cpu_runtime"]
    for name in [
...
def test_full_canonical_compact_exports_are_byte_identical(tmp_path: Path) -> None:
    ...
    for root, compact, hash_seed in zip(
        roots, compact_roots, ("101", "202"), strict=True
    ):
        environment = {**os.environ, "PYTHONHASHSEED": hash_seed}
        subprocess.run(
            [
