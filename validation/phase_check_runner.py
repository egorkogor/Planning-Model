...
from validation.model_training_report_validator import validate_all_training_evidence, validate_model_audit_report
...
def core_check(kind: str, phase: str, check_id: str, report: dict) -> list[str]:
    ...
    if kind == "bootstrap_manifest":
        ...
        if not path.is_file(): errors.append("scope artifact missing")
            ...
            if "work-planner/1.21" not in text or "runbook 2.21" not in text: errors.append("scope version markers missing")
        ...
        try:
            errors.extend(validate_all_training_evidence(ROOT))
            for directory in (ROOT / "reports/training/final", ROOT / "reports/training/flops-sensitivity"):
